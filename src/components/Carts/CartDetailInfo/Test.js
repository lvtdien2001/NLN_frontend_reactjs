...
const CartDetailInfo = ({ data, names }) =>{
    ...
    for (let i=0; i<data.length; i++){
        initCheckeds.push(false);
    }
    const [checkeds, setCheckeds] = useState(initCheckeds);
    ...
    const EmptyProduct = () => {
        return (
            <div className={`${cx('emptyWrapper')}`}>
                <p className='emptyIcon'>
                    <BsEmojiLaughing style={{width: '14rem', height: '14rem'}} />
                </p>
    ...
    return (
        <div className={cx('wrapper')}>
            ...
            {data.length===0 ? <EmptyProduct /> : <CartContent />}
